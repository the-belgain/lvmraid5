...
# TODO: move the non-standard imports to check_dependencies and given more
# helpful errors.
import pexpect
...
class HardDrive(LvmRaidBaseClass):
    ...
    def create_partition(self, size):
        ...
        # Set the partition's type to be raid-autodetect.
...
class RaidArray(LvmRaidBaseClass):
    ...
    def get_info(self):
        ...
        self.state = RaidArray.state_re.search(output).group('state')
...
class LvmRaidExec:
    ...
    def __init__(self, args):
        ...
        add_parser = subparsers.add_parser(
            'add',
            help="""Add a new drive to an existing array.
    ...
    def examine(self):
        ...
        self.log('Examining volume {}'.format(self.args.filesystem),
                 logging.INFO)
        lv = LogicalVolume(self.args.filesystem)
        print(lv)
    ...
    def remove(self):
        ...
        drive = HardDrive.find_or_create(drive_to_remove)
    ...
    def add(self):
        ...
        check_critical(
            lv.is_clean(),
            """Drives can only be added to non-degraded arrays.
            To replace a failed drive in the array,
            use the 'replace' command.""")
        check_critical((new_drive.size() > lv.largest_drive_size()) or
                       (new_drive.size() in lv.drive_sizes()),
        ...
        arrays = sorted([pv.raid_array for pv in lv.lv.pvs.values()],
                        key=lambda element: element.members_size,
                        reverse=True)
        for array in arrays:
            member = new_drive.create_partition(array.member_size)
            if not member:
        ...
        if new_drive.unallocated_size > 0:
            for drive in lv.vg.drives():
                if drive.unallocated_size > 0:
